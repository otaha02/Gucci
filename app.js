const stages=[...document.querySelectorAll('#lessonView .lesson-panel > .stage')];
...
state.stage=Number.isInteger(state.stage)?Math.max(0,Math.min(7,state.stage)):0;
state.learn=Number.isInteger(state.learn)?state.learn:0;
state.build=Number.isInteger(state.build)?state.build:0;
state.speaking=Number.isInteger(state.speaking)?state.speaking:0;
state.conversation=Number.isInteger(state.conversation)?state.conversation:0;
...
// Writing
function normalize(s){return s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[’‘`´]/g,"'").replace(/[.!?,;:]+$/g,'').replace(/\s+/g,' ').trim();}
function conversationIsCorrect(idx,text){const t=normalize(text);if(conversation[idx].answers.some(a=>t===normalize(a)))return true;const patterns=[[/\b(m'appelle|m appelle|moi,? c'est|moi c'est|je suis)\b.*\bsheila\b/], [/\b(j'habite|j habite|habite)\b.*\bsaint[- ]didier\b/], [/\b(j'ai|j ai|oui.*j'ai|oui.*j ai)\b.*\btrois\b.*\benfants?\b/], [/\b(j'aime|j aime|aime)\b.*\bcuisiner\b/]];return patterns[idx]?patterns[idx].some(r=>r.test(t)):false;}
...
// Conversation
document.getElementById('conversationCheck').addEventListener('click',()=>{const input=document.getElementById('conversationAnswer');if(!input||!input.value.trim()){document.getElementById('conversationMessage').textContent='Write your answer first.';return;}const idx=state.conversation;const item=conversation[idx];const ok=conversationIsCorrect(idx,input.value);const msg=document.getElementById('conversationMessage');if(ok){msg.textContent='✓ Great answer!';}else{msg.innerHTML='Not quite. A good answer is: <b>'+item.answers[0]+'</b>';};state.conversation++;document.getElementById('conversationCheck').hidden=true;document.getElementById('conversationNext').hidden=false;save();});
